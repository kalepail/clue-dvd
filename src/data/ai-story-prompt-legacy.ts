...
export const buildStoryUserPrompt = (params: {
  ...
  const extractPlaceholders = (template: string) =>
    Array.from(template.matchAll(/\{([a-zA-Z0-9_]+)\}/g)).map((match) => match[1]);

  const formatBind = (template: string, bind: Record<string, string>) => {
    const allowed = new Set(extractPlaceholders(template));
    const parts = Object.entries(bind)
      .filter(([key]) => allowed.has(key))
      .map(([key, value]) => `${key}=${value === "solution" ? "sol" : "non"}`);
  ...
  const formatBundle = (bundle: string[]) =>
    bundle
      .map((id) => {
        ...
        return `${entry.template}${formatBind(entry.template, entry.solutionBind)}`;
      })
      .join(" | ");

  return `Generate a full mystery package using the provided bundles and elements. Do not invent mechanics outside the bundles. Use the template wording verbatim except to replace placeholders with provided elements; do not swap in gendered pronouns.
All clues must be mutually consistent and compatible with the answer key. You must choose placeholder values that make every clue true in the same story world.
...
You may add brief connective phrasing only if it does not change the template meaning.
